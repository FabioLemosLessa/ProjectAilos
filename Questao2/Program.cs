...
public class Program
{
    ...
    public static int getTotalScoredGoals(string team, int year)
    {
        ...
        using (var clientRest = CriaRestClient(string.Format("year={0}&team1={1}", year, team)))
        {
            ...
            if (response != null)
            {
                dynamic res = JsonConvert.DeserializeObject<ExpandoObject>(response.Content);
                totalpages = ((IDictionary<string, object>)res).ContainsKey("total_pages") ? ((int)res.total_pages) : 0;
            }
        }
